...
router.post('/comprehensive-planning', auth.verifyToken, async (req, res) => {
  try {
    ...
    const parseAgentResult = (agentResult) => {
      if (!agentResult?.data?.result) return null;
      
      try {
        // If result is already an object, return it
        if (typeof agentResult.data.result === 'object') {
          return agentResult.data.result;
        }
        
        // If result is a string, try to parse it as JSON
        if (typeof agentResult.data.result === 'string') {
          return JSON.parse(agentResult.data.result);
        }
        
        return agentResult.data.result;
      } catch (error) {
        console.error('Error parsing agent result:', error);
        return agentResult.data.result; // Return raw result if parsing fails
      }
    };

    // Parse and structure agent results
    const parsedBudgetAnalysis = parseAgentResult(budget_analyst);
    const parsedDestinationResearch = parseAgentResult(destination_research);
    const parsedItineraryPlanning = parseAgentResult(itinerary_planning);
    const parsedTravelCoordination = parseAgentResult(travel_coordinator);

    res.json({
      success: true,
      data: {
        budgetAnalysis: parsedBudgetAnalysis,
        destinationResearch: parsedDestinationResearch,
        itineraryPlanning: parsedItineraryPlanning,
        travelCoordination: parsedTravelCoordination,
