...
class AgentSystem {
  constructor() {
    ...
    this.openai = new OpenAI({
      ...
      timeout: config.timeout || 120000 // Extended timeout for complex agent tasks (2 minutes)
    });
  ...
  getItineraryPlanningAgent() {
    return {
      ...
      CRITICAL: You MUST generate itineraries for the EXACT number of days specified in the trip duration.
      If the trip is 4 days, create day1, day2, day3, day4.
      If the trip is 7 days, create day1 through day7.
      If the trip is 14 days, create day1 through day14.
      ...
      IMPORTANT: You MUST respond with valid JSON in this exact format:
      {
        ...
        "routeOptimization": {
          "transportationStrategy": "Public transport recommendations",
          "walkingRoutes": "Suggested walking paths",
          "timeOptimization": "Tips for efficient timing"
        },
        "budgetBreakdown": {
          "dailyBudgets": [150, 200, 180, 170, ...],
          "totalItineraryCost": 700,
          "savingsOpportunities": ["Tip 1", "Tip 2"]
        },
        "alternativePlans": {
          "rainDay": "What to do if it rains",
          "budgetCuts": "How to reduce costs",
          "timeExtensions": "What to add if you have more time"
        },
        "practicalTips": [
   ...
   * ENHANCED with comprehensive JSON parsing and fallback handling
   */
  async executeAgentTask(agentType, task, context = {}) {
    try {
      ...
      console.log(`🤖 Executing ${agentType} agent task...`);
      ...
      const completion = await this.openai.chat.completions.create({
        ...
        max_tokens: this.config.maxTokens || 4000 // Increased token limit for detailed itineraries
        // Removed response_format to ensure compatibility with all OpenAI models
      });

      const response = completion.choices[0].message.content;
      console.log(`📝 ${agentType} raw response length:`, response?.length || 0);
      
      let parsedResponse;
      try {
        parsedResponse = JSON.parse(response);
        console.log(`✅ ${agentType} JSON parsed successfully`);
      } catch (parseError) {
        console.error(`❌ JSON parse error for ${agentType}:`, parseError);
        console.error(`Raw response: ${response?.substring(0, 500)}...`);
        
        // Create a comprehensive fallback response for itinerary planning
        if (agentType === 'itinerary_planning') {
          const duration = context.duration || 4;
          const dailyBudget = (context.budget || 1000) / duration;
          const startDate = new Date(context.travelDates?.start || new Date());
          
          // Generate fallback itinerary for all days
          const fallbackDailyItinerary = {};
          for (let i = 1; i <= duration; i++) {
            const currentDate = new Date(startDate);
            currentDate.setDate(startDate.getDate() + (i - 1));
            
            fallbackDailyItinerary[`day${i}`] = {
              date: currentDate.toISOString().split('T')[0],
              theme: i === 1 ? "Arrival and Orientation" : 
                     i === duration ? "Departure and Final Exploration" : 
                     `${context.destination || 'Destination'} Exploration Day ${i}`,
              activities: [
                {
                  time: "09:00",
                  activity: i === 1 ? "Arrival and hotel check-in" : 
                           i === duration ? "Check-out and departure preparation" :
                           `Explore ${context.destination || 'destination'} attractions`,
                  location: context.destination || "Destination",
                  duration: "2-3 hours",
                  cost: i === 1 || i === duration ? 0 : Math.round(dailyBudget * 0.4),
                  description: i === 1 ? "Settle in and get oriented" :
                              i === duration ? "Prepare for departure" :
                              "Discover local attractions and culture",
                  tips: i === 1 ? "Rest and plan your trip" :
                       i === duration ? "Allow extra time for travel" :
                       "Book tickets in advance when possible"
                },
                {
                  time: "14:00",
                  activity: i === 1 ? "Local area exploration" :
                           i === duration ? "Last-minute shopping" :
                           "Cultural activities and dining",
                  location: `${context.destination || 'Destination'} city center`,
                  duration: "3-4 hours",
                  cost: Math.round(dailyBudget * 0.3),
                  description: "Immerse in local culture and cuisine",
                  tips: "Try local specialties and interact with locals"
                }
              ],
              totalDayCost: Math.round(dailyBudget),
              transportationNotes: "Use public transportation for cost efficiency",
              mealSuggestions: ["Local breakfast spot", "Traditional lunch venue", "Authentic dinner restaurant"]
            };
          }
          
          parsedResponse = {
            error: 'Failed to parse detailed itinerary response - using fallback',
            fallbackItinerary: {
              dailyItinerary: fallbackDailyItinerary,
              routeOptimization: {
                transportationStrategy: "Prioritize public transport and walking",
                walkingRoutes: "Plan walking routes between nearby attractions",
                timeOptimization: "Visit popular attractions early morning or late afternoon"
              },
              budgetBreakdown: {
                dailyBudgets: Array(duration).fill(Math.round(dailyBudget)),
                totalItineraryCost: Math.round((context.budget || 1000) * 0.7),
              ...
              alternativePlans: {
                rainDay: "Visit museums, indoor markets, cafes, and covered shopping areas",
                budgetCuts: "Focus on free attractions, cook some meals, use public transport exclusively",
                timeExtensions: "Add day trips to nearby cities, explore more neighborhoods, take cooking classes"
              },
              practicalTips: [
            ...
            fallbackReason: 'JSON parsing failed - generated comprehensive fallback itinerary'
          };
        } else {
          parsedResponse = {
  ...
  buildAgentPrompt(agent, task, context) {
    ...
    let startDate = travelDates?.start;
    let endDate = travelDates?.end;
...
- Total Budget: $${budget}
- Trip Duration: ${duration} days (MUST create itinerary for ALL ${duration} days)
- Number of Travelers: ${travelers}
- Travel Dates: ${startDate} to ${endDate}
- Daily Budget: $${Math.round(budget / duration)}

**DATE BREAKDOWN:**
${dateList.map((date, index) => `Day ${index + 1}: ${date}`).join('\n')}

**TRAVELER PREFERENCES:**
- Interests: ${interests.join(', ')}
- Budget Priority: ${preferences.budgetPriority || 'medium'}
...
**AGENT-SPECIFIC INSTRUCTIONS:**`;
    ...
    switch (agent.role) {
      case 'Budget Analyst':
        prompt += `
- Focus on cost optimization and budget allocation
- Provide detailed breakdown of expenses
- Suggest money-saving alternatives
- Calculate daily budget: $${Math.round(budget / duration)}`;
        break;
        
      case 'Destination Research Specialist':
        prompt += `
- Research ${destination} thoroughly
- Find budget-friendly attractions and activities
- Provide cultural insights and local tips
- Consider seasonal factors and weather`;
        break;
        
      case 'Itinerary Planning Specialist':
        prompt += `
- Create detailed day-by-day itinerary for ALL ${duration} days
- Each day should have 3-5 activities with specific times
- Include realistic travel times between locations
- Balance must-see attractions with local experiences
- Ensure total itinerary cost stays within budget
- CRITICAL: Generate exactly ${duration} days (day1 through day${duration})
- Use the provided dates: ${dateList.join(', ')}`;
        break;
        
      case 'Travel Coordinator':
        prompt += `
- Provide booking strategies and logistics
...
- Ensure data is realistic and actionable
