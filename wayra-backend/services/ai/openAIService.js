...
class OpenAIService {
  constructor() {
    ...
    this.openai = new OpenAI({
      ...
      timeout: config.timeout || 120000 // Increased timeout to 120 seconds
    });
  ...
  async generateTravelPlan(request) {
    try {
      ...
      // Generate travel plan using OpenAI with retry logic
      const completion = await this._retryOpenAICall(async () => {
      ...
      if (error.name === 'APIConnectionTimeoutError' || error.message.includes('timeout')) {
        ...
        errorMessage = 'The AI service is taking longer than expected. Please try again.';
   ...
   */
  async _retryOpenAICall(apiCall, maxRetries = 3) {
    let lastError;
